...
def shop_user_purchases():
    try:
        ...
        # 取得重置後的購買記錄
        purchases_data = get_user_purchases(user_id)
...
def get_user_id_from_token():
    try:
        ...
        # 從Authorization header獲取Bearer token
        auth_header = request.headers.get('Authorization')
...
def can_purchase_item(item, user_purchases, user_items, user_level):
    item_id = item["id"]
    item_purchases = user_purchases.get("purchases", {}).get(item_id, {})
    ...
    # 檢查總限購
    if item.get("limit_per_account", -1) > -1:
        total_purchased = item_purchases.get("total_purchased", 0)
        if total_purchased >= item["limit_per_account"]:
            return False, "已達帳號總限購數量"
    
    # 檢查重置週期限購
    reset_type = item.get("reset_type", "none")
    if reset_type != "none" and item.get("limit_per_reset", -1) > 0:
        reset_key = f"{reset_type}_purchased"
        reset_purchased = item_purchases.get(reset_key, 0)
        if reset_purchased >= item["limit_per_reset"]:
            return False, f"已達{reset_type}限購數量"
    
    # 檢查道具是否足夠
    item_cost = item.get("cost", {})
    for cost_item, cost_amount in item_cost.items():
        owned = user_items.get(cost_item, 0)
        if owned < cost_amount:
            return False, f"{cost_item} 數量不足"
    
    # 檢查目標道具是否會超過999
    if item.get("type") != "bundle" and item.get("item_id"):
        current_amount = user_items.get(item["item_id"], 0)
        if current_amount + item.get("quantity", 1) > 999:
            return False, "購買後會超過999個上限"
    
    return True, ""

def process_purchase(item, user_id, user_purchases):
    db = firestore.client()
    
    # 更新購買記錄
    item_id = item["id"]
    if item_id not in user_purchases["purchases"]:
        user_purchases["purchases"][item_id] = {
            "total_purchased": 0,
            "daily_purchased": 0,
            "weekly_purchased": 0,
            "monthly_purchased": 0
    ...
    # 增加購買計數
    user_purchases["purchases"][item_id]["total_purchased"] += 1
    
    reset_type = item.get("reset_type", "none")
    if reset_type != "none":
        reset_key = f"{reset_type}_purchased"
        user_purchases["purchases"][item_id][reset_key] += 1
    
    # 扣除消費道具
    user_items_ref = db.collection("user_items").document(user_id)
    user_items_doc = user_items_ref.get()
    user_items = user_items_doc.to_dict() if user_items_doc.exists else {"id": user_id, "items": {}}
    
    item_cost = item.get("cost", {})
    for cost_item, cost_amount in item_cost.items():
        current_amount = user_items.get("items", {}).get(cost_item, 0)
        user_items["items"][cost_item] = max(0, current_amount - cost_amount)
    ...
    if item.get("type") == "bundle":
        ...
        for bundle_item in item.get("items", []):
            bundle_item_id = bundle_item["item_id"]
            bundle_quantity = bundle_item["quantity"]
            
            current_amount = user_items.get("items", {}).get(bundle_item_id, 0)
            new_amount = min(999, current_amount + bundle_quantity)
            user_items["items"][bundle_item_id] = new_amount
            
            items_received[bundle_item_id] = new_amount - current_amount
    else:
        # 單一道具
        target_item_id = item.get("item_id")
        target_quantity = item.get("quantity", 1)
        
        if target_item_id:
            current_amount = user_items.get("items", {}).get(target_item_id, 0)
            new_amount = min(999, current_amount + target_quantity)
            user_items["items"][target_item_id] = new_amount
            
            items_received[target_item_id] = new_amount - current_amount
    
    # 保存更新
    user_items_ref.set(user_items)
    save_user_purchases(user_id, user_purchases)
    
    return items_received, user_items
...
def update_shop_visit_time():
    ...
    try:
        ...
        if not visit_time:
            return jsonify({'error': '缺少時間戳'}), 400
        ...
        purchases_ref.set({
            'user_id': user_id,
            'last_shop_visit_time': visit_time,
...
def save_reset_purchases():
    ...
    try:
        ...
        if not purchases_data:
            return jsonify({'error': '缺少購買資料'}), 400
...
def format_week_string(dt):
    year = dt.year
    week_num = dt.isocalendar()[1]  # ISO週數
    return f"{year}-W{week_num:02d}"

def format_month_string(dt):
    return dt.strftime('%Y-%m')

def check_reset_needed(last_visit_time):
    if not last_visit_time:
        return {"daily": False, "weekly": False, "monthly": False}
    
    last_time = datetime.fromtimestamp(last_visit_time, tz=pytz.timezone('Asia/Taipei'))
    current_time = get_taipei_time()
    
    # 比較日期/週/月
    last_date = format_date_string(last_time)
    current_date = format_date_string(current_time)
    
    last_week = format_week_string(last_time)
    current_week = format_week_string(current_time)
    
    last_month = format_month_string(last_time)
    current_month = format_month_string(current_time)
...
def get_user_purchases(user_id):
    db = firestore.client()
    doc_ref = db.collection("shop_purchases").document(user_id)
    doc = doc_ref.get()
...
def save_user_purchases(user_id, purchases_data):
    db = firestore.client()
    doc_ref = db.collection("shop_purchases").document(user_id)
    doc_ref.set(purchases_data)

# 執行重置邏輯
def apply_resets(purchases_data, reset_flags):
    reset_items = []
    ...
    for item in shop_items:
        ...
        if reset_type in reset_flags and reset_flags[reset_type]:
            # 初始化商品購買記錄（如果不存在）
            ...
            # 重置對應的購買計數
            if reset_type == "daily":
                purchases_data["purchases"][item_id]["daily_purchased"] = 0
                reset_items.append(f"{item_name} (每日)")
            elif reset_type == "weekly":
                purchases_data["purchases"][item_id]["weekly_purchased"] = 0
                reset_items.append(f"{item_name} (每週)")
            elif reset_type == "monthly":
                purchases_data["purchases"][item_id]["monthly_purchased"] = 0
                reset_items.append(f"{item_name} (每月)")
    
    return reset_items

@app.route('/shop_check_reset', methods=['POST'])
def shop_check_reset():
    try:
        # 獲取用戶ID
        user_id = get_user_id_from_token()
        if not user_id:
        ...
        # 檢查是否需要重置
        last_visit_time = purchases_data.get("last_visit_time", 0)
        reset_flags = check_reset_needed(last_visit_time)
        
        reset_items = []
        reset_occurred = any(reset_flags.values())
        
        if reset_occurred:
            print(f"🔄 用戶 {user_id} 需要重置: {reset_flags}")
            # 執行重置
            reset_items = apply_resets(purchases_data, reset_flags)
        
        # 更新最後進入時間
        purchases_data["last_visit_time"] = get_taipei_time().timestamp()
        
        # 保存更新後的記錄
        save_user_purchases(user_id, purchases_data)
...
class ShopTimeValidator:
    """商店購買時間驗證器"""
    
    def __init__(self):
        self.db = firestore.client()
    
    def validate_purchase_time_restriction(self, item_data, user_purchases, user_level):
        """
        驗證購買時間限制
        
        Args:
            item_data: 商品資料
            user_purchases: 使用者購買記錄
            user_level: 使用者等級
        ...
        print(f"🔍 後端驗證購買：{item_data.get('name')} (ID: {item_id})")
        print(f"📊 購買記錄: {item_purchases}")
        ...
        if user_level < required_level:
            return {
                'valid': False,
                'error': f'等級不足 (需要Lv.{required_level}，目前Lv.{user_level})',
                'error_type': 'LEVEL_INSUFFICIENT'
            }
        
        # 2. 帳號總限購檢查
        limit_per_account = item_data.get('limit_per_account', -1)
        total_purchased = item_purchases.get('total_purchased', 0)
        
        if limit_per_account > -1 and total_purchased >= limit_per_account:
            return {
                'valid': False,
                'error': f'已達帳號總限購數量 ({total_purchased}/{limit_per_account})',
                'error_type': 'ACCOUNT_LIMIT_REACHED'
            }
        
        # 3. 重置週期限購檢查
        reset_type = item_data.get('reset_type', 'none')
        limit_per_reset = item_data.get('limit_per_reset', -1)
        
        if reset_type != 'none' and limit_per_reset > 0:
            time_check = self._validate_reset_period(
                reset_type, limit_per_reset, item_purchases, current_time
            )
        ...
        return {
            'valid': True,
            'current_periods': {
                'date': format_date_string(current_time),
                'week': format_week_string(current_time),
                'month': format_month_string(current_time)
            }
        }
    
    def _validate_reset_period(self, reset_type, limit_per_reset, item_purchases, current_time):
        """驗證重置週期限制"""
        current_date = format_date_string(current_time)
        current_week = format_week_string(current_time)
        current_month = format_month_string(current_time)
        
        print(f"⏰ 後端檢查重置週期：{reset_type}")
        print(f"📅 當前時間週期 - 日:{current_date} 週:{current_week} 月:{current_month}")
        
        if reset_type == 'daily':
            last_daily = item_purchases.get('last_daily_period', '')
            daily_purchased = item_purchases.get('daily_purchased', 0)
            
            print(f"📅 每日檢查 - 上次週期:{last_daily} 購買次數:{daily_purchased}/{limit_per_reset}")
            
            if last_daily == current_date and daily_purchased >= limit_per_reset:
                next_reset = current_time.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
                return {
                    'valid': False,
                    'error': f'今日已達購買上限 ({daily_purchased}/{limit_per_reset})',
                    'error_type': 'DAILY_LIMIT_REACHED',
                    'next_reset_time': next_reset.timestamp() * 1000
                }
        
        elif reset_type == 'weekly':
            last_weekly = item_purchases.get('last_weekly_period', '')
            weekly_purchased = item_purchases.get('weekly_purchased', 0)
            
            print(f"📅 每週檢查 - 上次週期:{last_weekly} 購買次數:{weekly_purchased}/{limit_per_reset}")
            
            if last_weekly == current_week and weekly_purchased >= limit_per_reset:
                # 計算下週一 00:00
                days_ahead = 7 - current_time.weekday()
                if days_ahead == 7:  # 如果今天就是週一
                    days_ahead = 7
                next_reset = (current_time + timedelta(days=days_ahead)).replace(
                    hour=0, minute=0, second=0, microsecond=0
                )
                return {
                    'valid': False,
                    'error': f'本週已達購買上限 ({weekly_purchased}/{limit_per_reset})',
            ...
            last_monthly = item_purchases.get('last_monthly_period', '')
            monthly_purchased = item_purchases.get('monthly_purchased', 0)
            
            print(f"📅 每月檢查 - 上次週期:{last_monthly} 購買次數:{monthly_purchased}/{limit_per_reset}")
            
            if last_monthly == current_month and monthly_purchased >= limit_per_reset:
                # 計算下個月1號 00:00
                ...
                return {
                    'valid': False,
                    'error': f'本月已達購買上限 ({monthly_purchased}/{limit_per_reset})',
                    'error_type': 'MONTHLY_LIMIT_REACHED',
                    'next_reset_time': next_reset.timestamp() * 1000
                }
        
        return {'valid': True}
    
    def update_purchase_record(self, item_data, user_purchases, purchase_time_ms=None):
        """
        更新購買記錄（含時間處理）
        ...
        Args:
            item_data: 商品資料
            user_purchases: 使用者購買記錄
            purchase_time_ms: 購買時間戳（毫秒），如為None則使用當前時間
        ...
        if purchase_time_ms:
            # 使用前端傳來的時間戳
            purchase_time = datetime.fromtimestamp(purchase_time_ms / 1000, TAIPEI_TZ)
        else:
            # 使用後端當前時間
            purchase_time = get_taipei_time()
        ...
        current_date = format_date_string(purchase_time)
        current_week = format_week_string(purchase_time)
        current_month = format_month_string(purchase_time)
        
        print(f"💰 後端處理購買：{item_data.get('name')}")
        print(f"⏰ 購買時間：{purchase_time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        
        # 更新總購買次數
        item_purchases['total_purchased'] = item_purchases.get('total_purchased', 0) + 1
        item_purchases['last_purchase_time'] = int(purchase_time.timestamp() * 1000)
        
        # 根據重置類型更新對應計數器
        reset_type = item_data.get('reset_type', 'none')
        
        if reset_type == 'daily':
            if item_purchases.get('last_daily_period') != current_date:
                # 新的一天，重置計數器
                item_purchases['daily_purchased'] = 1
                item_purchases['last_daily_period'] = current_date
                print(f"🌅 新的一天，重置每日計數器")
            else:
                # 同一天內，增加計數
                item_purchases['daily_purchased'] = item_purchases.get('daily_purchased', 0) + 1
        
        elif reset_type == 'weekly':
            if item_purchases.get('last_weekly_period') != current_week:
                # 新的一週，重置計數器
                item_purchases['weekly_purchased'] = 1
                item_purchases['last_weekly_period'] = current_week
                print(f"📅 新的一週，重置每週計數器")
            else:
                # 同一週內，增加計數
                item_purchases['weekly_purchased'] = item_purchases.get('weekly_purchased', 0) + 1
        
        elif reset_type == 'monthly':
            if item_purchases.get('last_monthly_period') != current_month:
                # 新的一月，重置計數器
                item_purchases['monthly_purchased'] = 1
                item_purchases['last_monthly_period'] = current_month
                print(f"📆 新的一月，重置每月計數器")
            else:
                # 同一月內，增加計數
                item_purchases['monthly_purchased'] = item_purchases.get('monthly_purchased', 0) + 1
        
        # 更新購買記錄
        purchases[item_id] = item_purchases
        user_purchases['purchases'] = purchases
        user_purchases['last_update_time'] = int(purchase_time.timestamp() * 1000)
        
        print(f"✅ 後端購買記錄已更新: {item_purchases}")
...
def shop_purchase():
    try:
        ...
        # 檢查是否可以購買
        can_buy, reason = can_purchase_item(item, user_purchases, user_items.get("items", {}), user_data.get("level", 1))
        
        if not can_buy:
            return jsonify({"success": False, "error": reason}), 400
        
        # 執行購買
        items_received, updated_user_items = process_purchase(item, user_id, user_purchases)
        
        return jsonify({
            "success": True,
            "message": "購買成功",
            "items_received": items_received,
            "user_items": updated_user_items.get("items", {}),
...
def shop_auto_reset_check():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "reset_applied": reset_result["reset_count"] > 0,
            "reset_count": reset_result["reset_count"],
            "reset_items": reset_result["reset_items"],
            "current_periods": reset_result["periods"],
            "check_time": time.time()
